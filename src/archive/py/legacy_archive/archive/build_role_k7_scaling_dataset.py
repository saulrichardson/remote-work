#!/usr/bin/env python3
"""
Build dataset measuring % growth in scaling by role_k7
Optimized for large LinkedIn dataset using DuckDB
"""
...
def build_role_scaling_dataset(input_path=None, output_path=None, sample_size=None):
    """
    Build a dataset showing % growth in employee count by role_k7
    
    This creates two outputs:
    1. Summary statistics by role and period
    2. Company-level growth rates for regression analysis
    """
    ...
    if output_path is None:
        output_path = "/Users/saul/Dropbox/Remote Work Startups/main/data/clean/role_k7_scaling_growth.csv"
    
    print("Building Role_k7 Scaling Growth Dataset")
    print("="*60)
    ...
    query = f"""
    -- First, create employee counts by company, role, and half-year
    ...
    period_counts AS (
        ...
        GROUP BY companyname, role_k7, year_half
    ),
    ...
    growth_rates AS (
        SELECT 
            ...
            LAG(employee_count) OVER (
                PARTITION BY companyname, role_k7 
                ORDER BY year_half
            ) as prev_count,
        ...
        SELECT 
            *,
            ...
            CASE WHEN total_employees_yh > 0
                 ...
                 , 6)
                 ELSE NULL END as d_role_share
        FROM growth_rates
        WHERE pct_growth_role IS NOT NULL
          AND employee_count >= 5  -- Filter out very small teams
    )
    
    SELECT * FROM labeled_growth
    ORDER BY companyname, role_k7, year_half
    """
    ...
    FROM read_csv_auto('{input_path}', strict_mode=false, null_padding=true, ignore_errors=true)
    WHERE companyname IS NOT NULL AND role_k7 IS NOT NULL
    """
    ...
    print(f"  - Unique roles: {df_growth['role_k7'].nunique()}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Build role_k7 scaling growth dataset')
