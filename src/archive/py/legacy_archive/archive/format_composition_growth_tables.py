#!/usr/bin/env python3
"""
Format productivity composition results with GROWTH controls into LaTeX tables
This uses growth rates (changes in shares) as controls, not share levels
"""
import pandas as pd
import numpy as np
...
def create_role_composition_table():
    """Create role composition effects table with GROWTH controls"""
    # Read role GROWTH control results
    ...
    for role in roles:
        ...
        if not role_df.empty:
            ...
            ols_inter = ols_df[ols_df['param'].str.contains('_inter', na=False)].iloc[0] if not ols_df[ols_df['param'].str.contains('_inter', na=False)].empty else None
            ...
            iv_inter = iv_df[iv_df['param'].str.contains('_inter', na=False)].iloc[0] if not iv_df[iv_df['param'].str.contains('_inter', na=False)].empty else None
            ...
            if ols_inter is not None:
                results[f'{role}_ols_inter_coef'] = ols_inter['coef']
                results[f'{role}_ols_inter_se'] = ols_inter['se']
                results[f'{role}_ols_inter_pval'] = ols_inter['pval']
            ...
            if iv_inter is not None:
                results[f'{role}_iv_inter_coef'] = iv_inter['coef']
                results[f'{role}_iv_inter_se'] = iv_inter['se']
                results[f'{role}_iv_inter_pval'] = iv_inter['pval']
...
def create_seniority_composition_table():
    """Create seniority composition effects table with GROWTH controls"""
    # Read seniority GROWTH control results
    ...
    for level in levels:
        ...
        if not level_df.empty:
            ...
            if ols_inter is not None:
                results[f'{level}_ols_inter_coef'] = ols_inter['coef']
                results[f'{level}_ols_inter_se'] = ols_inter['se']
                results[f'{level}_ols_inter_pval'] = ols_inter['pval']
            ...
            if iv_inter is not None:
                results[f'{level}_iv_inter_coef'] = iv_inter['coef']
                results[f'{level}_iv_inter_se'] = iv_inter['se']
                results[f'{level}_iv_inter_pval'] = iv_inter['pval']
...
def main():
    """Create LaTeX document with productivity composition tables - GROWTH control version"""
...
"""
    ...
    # Save to file
    output_path = '/Users/saul/Dropbox/Remote Work Startups/main/writeup/composition_effects_growth.tex'
    with open(output_path, 'w') as f:
        f.write(doc)
    
    print(f"LaTeX file saved to: {output_path}")
    print("To compile: cd writeup && pdflatex composition_effects_growth.tex")
