#!/usr/bin/env python3
"""
Create LaTeX table comparing OLS and IV horse race specifications
"""
...
\multicolumn{7}{p{0.95\textwidth}}{\footnotesize \textit{Notes:} All estimates include firm×user and time fixed effects with standard errors clustered by user in parentheses. 
Panel A shows OLS estimates, Panel B shows IV estimates using teleworkability as instruments.
Each column represents one regression specification. Columns 2-6 include interaction terms with high/low indicators based on median splits.
The first two rows in each panel show effects for the low group (below median growth/rent/HHI). 
...
# Write LaTeX file
output_file = f"{clean_results_dir}/horse_race_ols_iv_table.tex"
...
standalone_file = f"{clean_results_dir}/horse_race_ols_iv_standalone.tex"
