...
DEFAULT_INPUT = DATA_RAW / "Scoop_workers_positions.csv"
DEFAULT_OUTPUT = DATA_CLEAN / "eng_noneng_growth.csv"
...
def build_growth(input_path: Path, output_path: Path, year_start: int, year_end: int) -> None:
    input_path = require_file(
        ...
        nonempty=True,
        purpose="engineer/non-engineer raw positions input",
    )
    ...
    WITH positions AS (
        ...
        FROM read_csv_auto({_sql_quote(input_path)}, strict_mode=false, ignore_errors=true)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Path to Scoop_workers_positions.csv (default: {DEFAULT_INPUT})",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    build_growth(args.input, args.output, args.year_start, args.year_end)
