...
def _read_csv_flexible(path: Path) -> pd.DataFrame:
    """Load a CSV trying UTF-8 first, then falling back to latin-1.

    Some of the sample exports contain occasional non-UTF8 characters;
    rather than failing outright we degrade gracefully by attempting a
    latin-1 decode before ultimately raising the original error.
    """
...
def main(
    *,
    ...
    df_firms = _read_csv_flexible(firm_path)
    ...
    df_users = _read_csv_flexible(worker_path)
    panel_b = build_panel(
        df_users.copy(),
        VAR_MAP_B,
        NICE_B,
