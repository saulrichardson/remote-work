...
Historically ``create_user_productivity_table.py`` emitted a combined table
with Panel A (OLS) and Panel B (IV).  The write-up now needs these panels as
separate tables so that they can be placed independently (Table 2 = OLS,
Table 3 = IV).  This script re-uses the existing helpers from the combined
table generator to keep formatting consistent.
"""
