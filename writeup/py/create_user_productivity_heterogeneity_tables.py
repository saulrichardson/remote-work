...
``py/heterogeneity_table.py`` helper.  It merely maps the location of the raw
...
def run_one(csv: Path, out: Path, caption: str, label: str) -> None:
    if not csv.exists():
    ...
    cmd = [
        PY,
        str(PROJECT_ROOT / "py" / "heterogeneity_table.py"),
        str(csv),
        "--caption",
        caption,
        "--label",
...
def main() -> None:
    for t in TASKS:
        run_one(Path(t["csv"]), Path(t["out"]), t["caption"], t["label"])
