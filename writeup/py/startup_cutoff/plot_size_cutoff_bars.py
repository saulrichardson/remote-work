#!/usr/bin/env python3
"""Plot OLS startup-size interaction coefficients across size cutoffs."""
...
Y_LABEL = r"$\\beta_{ \text{Remote} \times \mathbf{1}(\text{Post}) \times \text{Startup (size)} }$"
...
PANELS: tuple[dict[str, str], ...] = (
    {
        ...
        "filename": "startup_size_cutoff_bars_total_contributions_q100.png",
    },
    {
        "panel": "Panel B",
        "dataset": "firm",
        ...
        "filename": "startup_size_cutoff_bars_growth_rate_we.png",
    },
)

COMBINED_FILENAME = "startup_size_cutoff_bars_main_panels.png"
CI_Z = 1.96
...
def _series(df: pd.DataFrame, *, outcome: str) -> pd.DataFrame:
    subset = df[(df["model_type"] == "OLS") & (df["param"] == "var5") & (df["outcome"] == outcome)].copy()
...
def _plot_coef_series(ax: plt.Axes, *, coefs: np.ndarray, ses: np.ndarray, show_ylabel: bool = True) -> None:
    x = np.arange(len(coefs), dtype=float)
    half = CI_Z * ses
    bars = ax.bar(x, coefs, width=0.55, color="#e0e0e0", edgecolor="#4a4a4a", linewidth=0.9, zorder=2)
    errs = np.vstack([half, half])
    kwargs = errorbar_kwargs("#1f1f1f").copy()
    kwargs.update({"fmt": "o", "linestyle": "none", "capsize": 4, "elinewidth": 1.2, "markersize": 5.3, "zorder": 3})
    ...
    ax.set_xlabel("Startup Size Cutoff (employees, 2019H2)")
...
def _single_figure(*, coefs: np.ndarray, ses: np.ndarray, output: Path) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE)
    _plot_coef_series(ax, coefs=coefs, ses=ses)
    apply_standard_figure_layout(fig)
    fig.savefig(output, bbox_inches="tight")
    plt.close(fig)


def _plot_panels(specs: Sequence[dict[str, str]], data: dict[str, tuple[np.ndarray, np.ndarray]], *, output: Path) -> None:
    ...
    for idx, (ax, spec) in enumerate(zip(axes, specs)):
        ...
        _plot_coef_series(ax, coefs=coefs, ses=ses, show_ylabel=(idx == 0))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the user size cutoff sweep CSV",
    )
    parser.add_argument(
        "--firm-csv",
        type=Path,
        default=RESULTS_RAW / "firm_scaling_size_cutoff_sweep" / "consolidated_results.csv",
        help="Path to the firm size cutoff sweep CSV",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    for spec in PANELS:
        ...
        cleaned_path = PROJECT_ROOT / "results" / "cleaned" / "startup_cutoff" / f"size_{spec['dataset']}_{spec['outcome']}_summary.csv"
        ensure_dir(cleaned_path.parent)
        cleaned.to_csv(cleaned_path, index=False)

        single_output = args.output_dir / spec["filename"]
        _single_figure(coefs=coefs, ses=ses, output=single_output)
    ...
    print(f"Saved two-panel startup size cutoff figure to {output_path}")
