#!/usr/bin/env python3
"""Render startup-definition robustness tables, one per outcome."""
...
def header_lines(label: str) -> list[str]:
    ...
    return [
        ...
        cmidrules,
        r"\textbf{Startup Age Cutoff} & " + numbering + LB,
        MID,
    ]
    ...
    for field, label in (("pre_mean", "Pre-Covid Mean"), ("nobs", "N")):
        ...
        for cut in CUTOFFS:
            ...
            if val.empty or pd.isna(val.iloc[0]):
                ...
                entries.append(
                    f"{int(round(val.iloc[0])):,}"
                    if field == "nobs"
                    else f"{val.iloc[0]:,.2f}"
                )
...
def build_table(
    *,
    ...
    lines: list[str] = [
        PREAMBLE_FLEX,
        rf"\begin{{tabular*}}{{\linewidth}}{{{column_format(len(CUTOFFS))}}}",
    ]
    ...
    for idx, (model, include_kp) in enumerate((("OLS", False), ("IV", True))):
        ...
        lines.append(
            rf"\multicolumn{{{len(CUTOFFS)+1}}}{{@{{}}l}}{{\textbf{{\uline{{{panel_label}}}}}}} {LB}"
        )
        lines.append(r"\addlinespace[2pt]")
...
def main() -> None:
    ...
    for dataset, outcome, label in OUTCOMES:
        ...
        output_path = args.output_dir / f"startup_cutoff_{outcome}.tex"
