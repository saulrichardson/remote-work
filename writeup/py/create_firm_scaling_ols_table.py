#!/usr/bin/env python3
"""Generate a two-panel OLS table for the Firm-Scaling specification."""
...
def build_panel_fe(df: pd.DataFrame) -> list[str]:
    ...
    for param in PARAM_ORDER:
        ...
        for tag in TAG_ORDER:
            sub = df.query("model_type=='OLS' and outcome=='growth_rate_we' and fe_tag==@tag and param==@param")
    ...
    lines.append(MID)
    lines.append(build_obs_row(df, TAG_ORDER, filter_expr="model_type=='OLS' and outcome=='growth_rate_we' and fe_tag=='{k}'"))
    return lines
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("% --------------------------------------------------------------")
    lines.append("%  Firm-Scaling: Two-panel OLS results")
    lines.append("% --------------------------------------------------------------")
    lines.append("")
    lines.append(r"\begin{table}[H]")
    lines.append(r"\centering")
    lines.append(r"\caption{Firm Scaling OLS}")
    lines.append(r"\label{tab:firm_scaling_ols}")
