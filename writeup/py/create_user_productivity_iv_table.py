...
def column_format(n: int) -> str:
    return "@{}l" + "".join([r"@{\hskip 8pt}>{\centering\arraybackslash}X" for _ in range(n)]) + "@{}"
...
def build_panel_fe(df: pd.DataFrame) -> list[str]:
    ...
    for param in PARAM_ORDER:
        ...
        for tag in TAG_ORDER:
            sub = df.query("model_type=='IV' and outcome=='total_contributions_q100' and fe_tag==@tag and param==@param")
    ...
    lines.append(MID)
    lines.append(build_obs_row(df, TAG_ORDER, filter_expr="model_type=='IV' and outcome=='total_contributions_q100' and fe_tag=='{k}'"))
    lines.append(build_kp_row(df, TAG_ORDER, filter_expr="model_type=='IV' and outcome=='total_contributions_q100' and fe_tag=='{k}'"))
    return lines
...
def main() -> None:
    ...
    lines.append(r"\caption{User Productivity -- IV}")
    lines.append(r"\label{tab:user_productivity_iv}")
