...
"""Build LaTeX table for wage-dispersion mechanism specs (user level)."""
...
SPECNAME = "user_mechanisms_wagegap"
INPUT_CSV = PROJECT_ROOT / "results" / "raw" / SPECNAME / "consolidated_results.csv"
OUTPUT_TEX = PROJECT_ROOT / "results" / "cleaned" / "user_mechanisms_wagegap.tex"
...
def main() -> None:
    ...
    lines.append(r"\caption{User Productivity – Wage Dispersion Mechanisms}")
    lines.append(r"\begin{tabular}{l" + "c" * len(SPEC_LIST) + "}")
    lines.append(r"\toprule")

    lines.append(r" & \multicolumn{%d}{c}{Total Contributions} \\" % len(SPEC_LIST))
    ...
    lines.append(r"\label{tab:user_mechanisms_wagegap}")
