...
lines.append(r"\caption{Firm Scaling Mechanisms}")
lines.append(r"\begin{tabular}{l" + "c"*len(specs) + "}")
lines.append(r"\toprule")

# ---------- NEW: group-header row -------------------------------------------
lines.append(
    r" & \multicolumn{%d}{c}{Growth} \\" % len(specs)
)
...
for panel_id, model in [("A", "OLS"), ("B", "IV")]:
    ...
    lines.append(r"\addlinespace")
    lines.append(r"Observations & " + " & ".join(nobs_vals) + r" \\")
...
lines.append(r"\label{tab:firm_mechanisms}")
