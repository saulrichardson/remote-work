#!/usr/bin/env python3
"""Build asset 12: user_productivity_precovid_stayer_table3.tex."""
...
from src.py.project_paths import RESULTS_CLEANED_TEX, RESULTS_RAW  # type: ignore
...
TIME_FE_INCLUDED = {"baseline_main_effect": True, "separate_fe": True}
...
def _stat_row(df: pd.DataFrame, model: str, columns: list[tuple[str, str]], label: str, field: str, fmt: str) -> str:
    cells: list[str] = []
...
def build_combined_table(df: pd.DataFrame, *, columns: list[tuple[str, str]], headers: dict[str, str]) -> str:
...
def load_stayer_results(variant: str) -> pd.DataFrame:
    root = RESULTS_RAW / f"12_user_productivity_{variant}_stayer_table3"
    init_path = root / "baseline_main_effect" / "consolidated_results.csv"
    alt_path = root / "interacted_columns" / "consolidated_results.csv"
    ...
    df_init["fe_tag"] = "baseline_main_effect"
...
def main() -> None:
    ...
    columns = [
        ("total_contributions_q100", "baseline_main_effect"),
        ("total_contributions_q100", "separate_fe"),
        ("total_contributions_we", "separate_fe"),
    ]
    headers = {
    ...
    filtered = filtered.replace(r"\hspace{1em}Half-year", r"\hspace{1em}Time")
