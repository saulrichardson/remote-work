#!/usr/bin/env python3
"""Build a 3-column robustness table: match FE + industry/state half-year shocks.

Inputs (from Stata spec):
  results/raw/user_productivity_industry_hqstate_shocks_<variant>/consolidated_results.csv

Output:
  results/cleaned/tex/user_productivity_<variant>_industry_hqstate_shocks.tex
"""
...
OUTCOME_LABEL = r"Contribution Rank"
...
def load_results(*, variant: str) -> pd.DataFrame:
    path = RESULTS_RAW / f"user_productivity_industry_hqstate_shocks_{variant}" / "consolidated_results.csv"
    if not path.exists():
...
def stat_rows(df: pd.DataFrame, *, model: str, include_kp: bool) -> list[str]:
    ...
    # Use the var3 row as the “header” for shared stats, since every column must
    # have var3 present (otherwise the table is not meaningful).
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="precovid",
        help="Which user_panel sample variant the Stata spec used (default: %(default)s).",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Destination TeX file (defaults to results/cleaned/tex/user_productivity_<variant>_industry_hqstate_shocks.tex).",
    )
...
def main() -> None:
    args = parse_args()
    df = load_results(variant=args.variant)
    df = df.loc[df["outcome"] == OUTCOME].copy()

    output_path = args.output
    if output_path is None:
        output_path = RESULTS_CLEANED_TEX / f"user_productivity_{args.variant}_industry_hqstate_shocks.tex"
    ensure_dir(output_path.parent)

    tex = build_table(df)
    output_path.write_text(tex)
    print(f"Wrote user shocks table → {output_path}")
