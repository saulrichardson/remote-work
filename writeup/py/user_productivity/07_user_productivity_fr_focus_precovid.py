...
def load_comparison(variant: str, suffix: str, fe_tag: str) -> dict:
    base_dir = RAW_DIR / f"07_user_productivity_fr_focus_{variant}" / suffix
    ...
    if "fe_tag" not in df.columns:
        raise ValueError(
            ...
            "Re-run spec/stata/tables/07_user_productivity_fr_focus_precovid.do to refresh outputs."
        )
...
def build_table(variant: str, comparisons: Iterable[str]) -> str:
    ...
    lines.append(
        r" & \multicolumn{" + f"{len(comparisons)}" + r"}{c}{Contribution Rank} " + E
    )
    lines.append(r"\cmidrule(lr){2-" + f"{len(comparisons) + 1}" + r"}")
    group_cells = [""]
    group_spans: list[tuple[int, int]] = []
    idx = 0
    while idx < len(comparisons):
        base_label = COMPARISON_COLUMNS[comparisons[idx]]["group"].replace("&", r"\&")
        label = base_label
        if base_label == "Hybrid/In-Person":
            label = r"\makecell[c]{Hybrid /\\In-Person}"
        span = 1
        while (
            idx + span < len(comparisons)
            and COMPARISON_COLUMNS[comparisons[idx + span]]["group"] == COMPARISON_COLUMNS[comparisons[idx]]["group"]
        ):
            span += 1
        group_spans.append((idx, span))
        if span == 1:
            group_cells.append(label)
        else:
            group_cells.append(rf"\multicolumn{{{span}}}{{c}}{{{label}}}")
        idx += span
    lines.append(" & ".join(group_cells) + f" {E}")
    col_start = 2
    for _, span in group_spans:
        col_end = col_start + span - 1
        lines.append(rf"\cmidrule(lr){{{col_start}-{col_end}}}")
        col_start = col_end + 1
    lines.append(
        r"\textbf{Sample Filter} & "
        + " & ".join(f"({i})" for i in range(1, len(comparisons) + 1))
        + f" {E}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Optional override for output path under results/cleaned/tex",
    )
