#!/usr/bin/env python3
"""Build asset 11: first_stage_summary.tex from owner-emitted diagnostics."""
...
USER_FIRST_STAGE_PATH = (
    RESULTS_RAW / "02_user_productivity_precovid_total_iv_single" / "first_stage" / "consolidated_results.csv"
)
FIRM_FIRST_STAGE_PATH = (
    RESULTS_RAW / "04_firm_scaling_precovid" / "first_stage" / "consolidated_results.csv"
)

OUTPUT_PATH = RESULTS_CLEANED_TEX / "first_stage_summary.tex"
...
def load_user_tables() -> dict[str, pd.DataFrame]:
    if not USER_FIRST_STAGE_PATH.exists():
        raise FileNotFoundError(USER_FIRST_STAGE_PATH)
    df = pd.read_csv(USER_FIRST_STAGE_PATH)
    df = df[df["outcome"] == TARGET_OUTCOME].copy()
    separate = df[df["fe_tag"] == "separate_fe"].copy()
    match = df[df["fe_tag"] == "match_fe"].copy()
    if separate.empty:
        raise ValueError("User first-stage export lacks separate_fe rows for total_contributions_q100")
    if match.empty:
        raise ValueError("User first-stage export lacks match_fe rows for total_contributions_q100")
    return {"separate_fe": separate, "match_fe": match}


def load_firm_table() -> pd.DataFrame:
    if not FIRM_FIRST_STAGE_PATH.exists():
        raise FileNotFoundError(FIRM_FIRST_STAGE_PATH)
    df = pd.read_csv(FIRM_FIRST_STAGE_PATH)
    df = df[(df["outcome"] == "growth_rate_we") & (df["fe_tag"] == "firm_time_fe")].copy()
    if df.empty:
        raise ValueError("Firm first-stage export lacks firm_time_fe rows for growth_rate_we")
    return df
...
def build_lines(user_tables: dict[str, pd.DataFrame], firm_df: pd.DataFrame) -> list[str]:
    ...
    # Use tabular* with filler glue between every numeric column so the table spans \linewidth
    colspec = "@{}l" + len(columns) * r"@{\extracolsep{\fill}}c" + "@{}"
