#!/usr/bin/env python3
"""Build Overleaf-style LaTeX table for the canonical keep-remote horse race."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build keep-remote horse-race table.")
    parser.add_argument(
        "--variant",
...
def build_table(df_base: pd.DataFrame, df_pair: pd.DataFrame) -> str:
    ...
    lines: list[str] = [
        "% Auto-generated block: keep-remote horse race",
        r"{\centering",
...
def main() -> None:
    ...
    base_path = RESULTS_RAW / f"user_horse_race_keep_remote_baseline_{variant}" / "consolidated_results.csv"
    pair_path = RESULTS_RAW / f"user_horse_race_keep_remote_pair_{variant}" / "consolidated_results.csv"
    out_path = RESULTS_CLEANED_TEX / f"user_horse_race_keep_remote_{variant}.tex"
