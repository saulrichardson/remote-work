...
def write_mechanisms_tables(
    *,
    ...
    output_tex_ols: Path | None = None,
    output_tex_iv: Path | None = None,
    ...
    for i in range(tables_needed):
        ...
        for df_model, out_lines, label in (
            (df_ols, lines_ols, "OLS"),
            (df_iv, lines_iv, "IV"),
        ):
            n_cols = len(chunk)
            col_spec = _tabular_star_spec(n_cols)
            header_nums = " & ".join(f"({j})" for j in range(1, n_cols + 1))
            out_lines.extend(
                [
                    f"% Auto-generated block: {label} (part {i + 1} of {tables_needed})",
                    r"{\centering",
                    rf"\begin{{tabular*}}{{\linewidth}}{{{col_spec}}}",
                    r"\toprule",
                    rf" & \multicolumn{{{n_cols}}}{{c}}{{Contribution Rank}} \\",
                    ...
                    " & " + header_nums + r" \\",
                    r"\midrule",
                    *panel_block(df_model, chunk),
                    r"\midrule",
                    *panel_stats_block(df_model, chunk, model=label),
                    r"\midrule",
                    *fixed_effects_block(n_cols, fe_row_labels),
                    r"\midrule",
                    *controls_block(chunk),
                    r"\bottomrule",
                    r"\end{tabular*}",
                    r"}",
                    "",
    ...
    if output_tex_ols is not None:
        output_tex_ols.write_text("\n".join(lines_ols).rstrip() + "\n", encoding="utf-8")
    if output_tex_iv is not None:
        output_tex_iv.write_text("\n".join(lines_iv).rstrip() + "\n", encoding="utf-8")
