...
COLUMN_CONFIG = [
    ...
    {
        "label": "(2)",
        "header": r"\makecell[c]{Total \\ Firm $\times$ Individual FE}",
        "fe_tag": "firmbyuseryh",
    ...
    {
        "label": "(4)",
        "header": r"\makecell[c]{Total \\ Individual + Firm FE}",
        "fe_tag": "fyhu",
        "outcome": "total_contributions_we",
    },
]
...
def build_panel(df: pd.DataFrame, model: str) -> str:
    ...
    for trait_key, cfg in TRAIT_CONFIG.items():
        ...
        for column in COLUMN_CONFIG:
            sub3 = subset[
                (subset["fe_tag"] == column["fe_tag"])
                & (subset["outcome"] == column["outcome"])
                & (subset["param"] == cfg["param_var3"])
            ]
            if sub3.empty:
                row_var3.append("")
            else:
...
def build_table(df: pd.DataFrame, model: str) -> str:
    header_nums = " & ".join([""] + [col["label"] for col in COLUMN_CONFIG]) + r" \\"
    header_desc = " & ".join([""] + [col["header"] for col in COLUMN_CONFIG]) + r" \\"
