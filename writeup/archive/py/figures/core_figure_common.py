...
"""Shared rendering helpers for the active core firm figures."""
...
try:
    ...
    raise SystemExit(
        ...
        "before executing the active core-figure builders."
    ) from exc
...
from project_paths import DATA_CLEAN, RESULTS_CLEANED_FIGURES, ensure_dir, require_file
...
def _read_dataset(path: Path) -> pd.DataFrame:
    """Read a canonical project dataset from CSV or Stata."""
...
# 3) File paths
WORKER_FILE = DATA_DIR / "user_panel_precovid.dta"
FIRM_FILE = DATA_DIR / "firm_panel.dta"
...
def load_core_inputs(*, firm_file: Path = FIRM_FILE, worker_file: Path = WORKER_FILE) -> tuple[pd.DataFrame, tuple[float, float]]:
    firms = _read_dataset(firm_file)
    _require_columns(
        firms,
        {"firm_id", "yh", "age", "remote", "teleworkable", "covid"},
        label=str(firm_file),
    )
...
def build_firm_age_lt100_remote(
    *,
    firm_file: Path = FIRM_FILE,
    worker_file: Path = WORKER_FILE,
) -> Path:
    firms_unique, remote_limits = load_core_inputs(
        firm_file=firm_file,
    ...
    output = OUTPUT_DIR / "firm_age_lt100_remote.png"
    print(f"Saved {output}")
    return output


def build_firm_teleworkable_remote(
    *,
    firm_file: Path = FIRM_FILE,
    worker_file: Path = WORKER_FILE,
) -> Path:
    ...
    output = OUTPUT_DIR / "firm_teleworkable_remote.png"
    print(f"Saved {output}")
    return output
...
if __name__ == "__main__":
    raise SystemExit(
        "Use writeup/py/figures/01_firm_age_lt100_remote.py or "
        "writeup/py/figures/02_firm_teleworkable_remote.py."
    )
