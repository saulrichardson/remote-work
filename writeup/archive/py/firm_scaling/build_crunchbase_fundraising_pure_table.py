#!/usr/bin/env python3
"""Format the *pure* Crunchbase fundraising spec (no startup terms) into LaTeX tables.

This corresponds to running:
  spec/stata/firm_scaling_crunchbase_fundraising.do pure

Compared to the canonical fundraising table builder, this script:
  - reads results from results/raw/firm_scaling_crunchbase_fundraising_pure/
  - shows only the Remote×Post coefficient (var3)
  - reports a meaningful "Share Zero" for rank/log outcomes by keying off
    cb_raised_usd==0 in the analysis sample, rather than y==0 on the derived rank.
...
Writes:
  - results/cleaned/tex/firm_scaling_crunchbase_fundraising_pure.tex
  - results/cleaned/tex/firm_scaling_crunchbase_fundraising_pure_additional.tex
"""
...
RAW_DIR = RESULTS_RAW / "firm_scaling_crunchbase_fundraising_pure"
...
def load_results() -> pd.DataFrame:
    if not RAW_RESULTS.exists():
        raise FileNotFoundError(
            ...
            "Run: do spec/stata/firm_scaling_crunchbase_fundraising.do pure"
        )
...
def load_panel_private() -> pd.DataFrame:
    ...
    # Derived round indicators (not always persisted by the builder).
...
def share_zero_by_outcome(panel: pd.DataFrame, outcomes: Sequence[str]) -> dict[str, float]:
    zero_source = {
        # These are derived from cb_raised_usd; share of zeros should refer to the
        # underlying dollars rather than the derived rank/log variable.
...
def build_table(results: pd.DataFrame, panel: pd.DataFrame, columns: Sequence[tuple[str, str]]) -> str:
    ...
    for panel_label, model in (("Panel A: OLS", "OLS"), ("Panel B: IV", "IV")):
        ...
        for outcome, _ in columns:
            rec = select_row(results, sample=MAIN_SAMPLE, outcome=outcome, model=model)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-main",
        type=Path,
        default=RESULTS_CLEANED_TEX / "firm_scaling_crunchbase_fundraising_pure.tex",
        help="Destination TeX file for main table.",
    )
    parser.add_argument(
        "--output-additional",
        type=Path,
        default=RESULTS_CLEANED_TEX / "firm_scaling_crunchbase_fundraising_pure_additional.tex",
