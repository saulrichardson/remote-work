#!/usr/bin/env python3
"""Shared rendering helpers for the active split firm-scaling tables."""
...
def load_cols1_4_data() -> pd.DataFrame:
    init = pd.read_csv(
        RAW_DIR / "04_firm_scaling_precovid_cols1_4" / "initial" / "consolidated_results.csv"
    )
    init["fe_tag"] = "init"

    base = pd.read_csv(
        RAW_DIR
        / "04_firm_scaling_precovid_cols1_4"
        / "growth_split"
        / "consolidated_results.csv"
    )
    base["fe_tag"] = "fyh"

    return pd.concat([init, base], ignore_index=True, sort=False)


def load_cols5_6_data() -> pd.DataFrame:
    vac = pd.read_csv(
        RAW_DIR / "05_firm_scaling_precovid_cols5_6" / "consolidated_results.csv"
    )
    vac["fe_tag"] = "vac"
    return vac
...
def build_cols1_4_table() -> Path:
    df = load_cols1_4_data()
    columns = [
    ...
    write_table(output, build_table(df, columns, start_index=1))
    return output


def build_cols5_6_table() -> Path:
    df = load_cols5_6_data()
    columns = [
...
if __name__ == "__main__":
    raise SystemExit(
        "Use writeup/py/firm_scaling/04_firm_scaling_precovid_cols1_4.py or "
        "writeup/py/firm_scaling/05_firm_scaling_precovid_cols5_6.py."
    )
