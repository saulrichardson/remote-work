#!/usr/bin/env python3
"""Build a non-duplicative sensitivity table for including Startup×Post (var4).

The meeting follow-up doc treats the baseline spec as *dropping* Startup×Post.
This script produces a compact table that reports the Remote×Post (var3) effect
under the alternative specification that *includes* Startup×Post as a control,
without repeating the dropped-var4 (baseline) estimates.
...
Writes:
  - results/cleaned/tex/firm_scaling_crunchbase_fundraising_var4_sensitivity.tex
"""
...
def load_panel_private() -> pd.DataFrame:
    ...
    for col in ["cb_round_count", "cb_raised_usd"]:
...
def share_zero_by_outcome(panel: pd.DataFrame, outcomes: Sequence[str]) -> dict[str, float]:
    zero_source = {
        # Derived from cb_raised_usd; use underlying dollars for zero-mass.
...
def build_table(results: pd.DataFrame, panel: pd.DataFrame) -> str:
    cols = COLUMNS
    share0 = share_zero_by_outcome(panel, [c for c, _ in cols])
    ncols = len(cols)
    ...
    for panel_label, model in (("Panel A: OLS", "OLS"), ("Panel B: IV", "IV")):
        ...
        # Parameter rows: show both var3 and var4 since var4 is included as a control in this spec.
        for param, label in ((PARAM, PARAM_LABEL), (VAR4, VAR4_LABEL)):
        ...
        n_cells = ["N"]
        for outcome, _ in cols:
            rec = select_row(results, sample=MAIN_SAMPLE, outcome=outcome, model=model, param=PARAM)
        ...
        if model == "IV":
            ...
            for outcome, _ in cols:
                rec = select_row(results, sample=MAIN_SAMPLE, outcome=outcome, model="IV", param=PARAM)
    ...
    lines.append(" & ".join([INDENT + "Half-Year"] + [r"$\checkmark$"] * ncols) + LB)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output",
        type=Path,
        default=RESULTS_CLEANED_TEX / "firm_scaling_crunchbase_fundraising_var4_sensitivity.tex",
        help="Destination TeX file for the sensitivity table.",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    args.output.write_text(build_table(results, panel), encoding="utf-8")
    print(f"Wrote sensitivity table → {args.output}")
