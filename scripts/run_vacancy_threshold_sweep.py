...
Example
  python scripts/run_vacancy_threshold_sweep.py \
    --input data/raw/vacancy/Postings_scoop.csv \
    --firm-panel data/samples/firm_panel.csv \
    --outdir data/processed/vacancy/sensitivity \
...
def main() -> None:
    ...
    build_script = root / "scripts" / "build_halfyear_panel.py"
    merge_script = root / "scripts" / "merge_halfyear_with_firm_panel.py"
    postp_script = root / "scripts" / "postprocess_halfyear_panel.py"
    ...
    multi_builder = root / "scripts" / "build_halfyear_panel_multi.py"
