...
def build_table(df: pd.DataFrame, *, caption: str, label: str) -> str:
    ...
    if "rkf" in df.columns:
        rkf_vals = [df.query("bucket == @b")["rkf"].iloc[0] for b in buckets]
        stats_rows.append("KP rk Wald F & " + " & ".join(f"{v:.2f}" for v in rkf_vals) + r" \\")

    # Assemble LaTeX -------------------------------------------------------
    header_cols = ["Parameter", *[str(b) for b in buckets]]
