...
#    • filtered_msa_cnt     – MSAs that meet ABS and REL floors **and** have cbsa
...
for chunk in tqdm(reader, total=None, desc="Reading", unit="chunk"):
    ...
    for row in chunk.itertuples(index=False):
        ...
        for yr, hf in half_span(s, e):
            presence[(row.companyname, yr, hf)][(cbsa, lat, lon)] += 1

# ── 3 ▸ apply ABS/REL filters and compute metrics ---------------------------
rows = []
for (comp, yr, hf), counter in tqdm(presence.items(), desc="Computing", unit="cmp"):
    total = sum(counter.values())
    coords = [ (lat, lon) for (cbsa, lat, lon), n in counter.items()
               if n >= ABS_FLOOR and n/total >= REL_FLOOR ]
