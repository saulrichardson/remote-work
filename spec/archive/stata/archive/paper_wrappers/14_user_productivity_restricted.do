* Table 14: restricted user productivity table
...
local __rc = _rc
if `__rc' {
    di as error "Underlying spec failed for table 14: $DIR_DO/user_productivity_initial.do (rc=`__rc')."
    exit `__rc'
}
...
local __rc = _rc
if `__rc' {
    di as error "Underlying spec failed for table 14: $DIR_DO/user_productivity_alternative_fe.do (rc=`__rc')."
    exit `__rc'
}
    ...
    "$results/user_productivity_alternative_fe_precovid/consolidated_results.csv" ///
{
    ...
    if _rc {
        di as error "Missing expected raw output for table 14: `f'"
        exit 601
    }
}
