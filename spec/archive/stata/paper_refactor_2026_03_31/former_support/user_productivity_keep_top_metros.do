*======================================================================*
* support/user_productivity_keep_top_metros.do
...
local stata_bin      "`pr_root'/bin/stata"
if !fileexists("`stata_bin'") {
    di as error "Stata wrapper not found: `stata_bin'"
    di as error "Expected repo wrapper at $PROJECT_ROOT/bin/stata."
    exit 601
}
...
foreach keep_count of local keep_targets {
    ...
    cd "`pr_root'"
    ! "`stata_bin'" -b do spec/stata/support/user_productivity_standard_total.do "`variant'"

    di as text "   Running firm×user FE spec for `variant'"
    ! "`stata_bin'" -b do spec/stata/support/user_productivity_firmbyuser.do "`variant'"
}
