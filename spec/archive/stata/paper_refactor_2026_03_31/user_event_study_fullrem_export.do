* Export user event-study coefficients using fully-remote indicator ------------
* Usage:  do spec/user_event_study_fullrem_export.do [panel_variant]

* 0) Parse optional panel variant argument (default = precovid)
args panel_variant
if "`panel_variant'" == "" local panel_variant "precovid"

local specname "user_event_study_fullrem_`panel_variant'"
...
local result_path_clean : subinstr local result_path "'" ""
local result_path "`result_path_clean'"
...
label var remote_full "Indicator for fully remote worker"
...
foreach outcome of local transformed_vars {
    foreach model of local models {
        ...
        format yh %th
        gen str12 period_label = subinstr(string(yh, "%th"), "h", "H", .)
        ...
        local fname_raw "`result_path'/`model'_`outcome'.csv'"
        local fname : subinstr local fname_raw "'" ""
