...
gen int  _year  = year(dofh(yh))
gen byte _half  = halfyear(dofh(yh))
gen str7 period_label = string(_year) + "H" + string(_half)
...
*---------------------------------------
* Run Regressions & SAVE COEFS (no plotting)
*---------------------------------------
...
foreach outcome of local transformed_vars {
    ...
    * Save coefficients to CSV (OLS)
    ...
    preserve
        ...
        order outcome estimator period period_label event_time omitted b lb ub yh yh_label
