...
*====================================================================*
*  spec/growth_mechanisms_both_fe.do
*  ------------------------------------------------------------------
*  Runs growth mechanisms analysis with two FE specifications:
*  1. firm_id#user_id yh (worker-firm fixed effects)
*  2. firm_id user_id yh (separate firm and user fixed effects)
*====================================================================*
...
local specname "growth_mechanisms_both_fe_`panel_variant'"
capture log close
cap mkdir "$LOG_DIR"
...
*--------------------------------------------------------------------*
* 6. Create residualized growth (matching line 281 of original)
*--------------------------------------------------------------------*
use `postcovid', clear
gen companyname_c = lower(companyname)
...
* Note: This creates fitted values (predicted growth), not residuals!

* Create EXOGENOUS growth tile (fitted values)
xtile tile_growth_resid = growth_resid, nq(2)

collapse (last) tile_growth_resid (last) tile_post_c, by(companyname)
...
gen var19 = covid * tile_growth_resid
gen var20 = covid * tile_growth_resid * startup

*--------------------------------------------------------------------*
* 8. Setup postfile for both FE specifications
*--------------------------------------------------------------------*
...
*--------------------------------------------------------------------*
* 10A. Run all specifications with firm#user_id FE
*--------------------------------------------------------------------*
local fe_spec "firm_id#user_id yh"
local fe_label "Worker-Firm FE"

* Specification 1: Baseline
di _n "=== `fe_label': Baseline ==="
ivreghdfe total_contributions_q100 (var3 var5 = var6 var7) var4, ///
    absorb(`fe_spec') vce(cluster user_id) savefirst
local rkf = e(rkf)
local N   = e(N)
foreach p in var3 var5 {
    ...
    post handle ("`fe_label'") ("IV") ("baseline") ("`p'") ///
                (`b') (`se') (`pval') (`pre_mean') ///
                (`rkf') (`N')
}

* Specification 2: Endogenous Growth
di _n "=== `fe_label': Endogenous Growth ==="
...
foreach p in var3 var5 {
    ...
    local t    = `b'/`se'
    local pval = 2*ttail(e(df_r), abs(`t'))
    post handle ("`fe_label'") ("IV") ("endo_growth") ("`p'") ///
                (`b') (`se') (`pval') (`pre_mean') ///
                (`rkf') (`N')
}

* Specification 3: Exogenous Growth
di _n "=== `fe_label': Exogenous Growth ==="
...
foreach p in var3 var5 {
    ...
    local t    = `b'/`se'
    local pval = 2*ttail(e(df_r), abs(`t'))
    post handle ("`fe_label'") ("IV") ("exo_growth") ("`p'") ///
                (`b') (`se') (`pval') (`pre_mean') ///
                (`rkf') (`N')
}

*--------------------------------------------------------------------*
* 10B. Run all specifications with separate firm_id user_id FE
*--------------------------------------------------------------------*
local fe_spec "firm_id user_id yh"
local fe_label "Separate FE"
