...
*============================================================*
* firm_scaling_role_growth.do
* ------------------------------------------------------------
* Baseline firm-scaling regressions (OLS + IV) by occupation.
* Uses pre-computed var3/var4/var5 controls from firm_panel.dta
* and growth rates from role_k7_scaling_growth.csv.
*============================================================*
...
// -----------------------------------------------------------------------------
local specname "firm_scaling_role_growth"
capture log close
cap mkdir "$LOG_DIR"
...
display as text "============================================================"
display as text "Running role-level firm scaling regressions"
...
// -----------------------------------------------------------------------------
// 3. Load occupation-level growth data and reshape to wide
// -----------------------------------------------------------------------------
import delimited "${processed_data}/role_k7_scaling_growth.csv", clear

// Keep the seven core K7 roles used across the project
...
// Prepare growth variables for reshape
...
display as text "Merged role growth data with firm panel: " _N " observations"
...
foreach r of local roles {
    ...
    display as text _n "→ Outcome: `yname'"

    count if !missing(`yvar')
    local sampleN = r(N)
    display as text "   Non-missing observations: `sampleN'"
    if (`sampleN' < 200) {
        display as error "   Skipping `yname' (insufficient support)"
        continue
    }
    ...
    if (_rc) {
        display as error "   OLS failed for `yname' (return code `_rc'). Skipping."
        continue
    }
    local N = e(N)
    ...
    if (_rc) {
        display as error "   IV failed for `yname' (return code `_rc'). Skipping IV output."
        continue
    }
...
export delimited using "`result_dir'/role_growth_var3_summary.csv", replace delimiter(",") quote

// -----------------------------------------------------------------------------
// 8. Close log
// -----------------------------------------------------------------------------
display as result _n "Role growth regressions written to:"
display as result "  - `result_dir'/consolidated_results.csv"
display as result "  - `result_dir'/first_stage.csv"
display as result "  - `result_dir'/role_growth_var3_summary.csv"
