...
*============================================================*
* spec/firm_scaling_mechanisms.do
*  Runs the baseline firm-scaling diff-in-diff (var3/var4/var5)
*  and augments it with rent, HHI, and seniority interactions.
...
foreach spec of local specs {
    ...
    foreach y of local outcome_vars {
        if "`filter'" == "" {
            ...
            quietly summarize `y' if covid == 0 & `filter', meanonly
        }
        if r(N) == 0 continue
        ...
        quietly reghdfe `y' `endo' `exog' `ifcmd', absorb(firm_id yh) vce(cluster firm_id)
        local N = e(N)
        foreach p of local params {
            ...
            post handle ("OLS") ("`spec'") ("`y'") ("`p'") ///
                (`b') (`se') (`pval') (`pre_mean') (.) (`N')
        }

        quietly ivreghdfe `y' (`endo' = `instr') `exog' `ifcmd', ///
