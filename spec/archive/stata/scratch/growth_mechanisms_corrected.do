...
*====================================================================*
*  Growth mechanisms analysis - CORRECTED MSA calculation
*  Following user_productivity_expost_growth_loop.do more closely
*====================================================================*
...
*--------------------------------------------------------------------*
* 4. EXOGENOUS GROWTH (residualized growth) - CORRECTED
*--------------------------------------------------------------------*
...
* Reload growth data and compute FIRM-LEVEL growth first
import delimited "$processed_data/Scoop_Positions_Firm_Collapse2.csv", clear
drop v1
gen date_numeric = date(date, "YMD")
drop date
...
* IMPORTANT: First collapse to firm-level average growth
preserve
    collapse (mean) fg_we, by(companyname industry company_msa)
    rename fg_we firm_growth_avg
    tempfile firm_growth
    save `firm_growth'
restore

* Now calculate leave-one-out means at FIRM level
...
* Now prepare for regression - get firm-level data with all variables
use `firm_growth', clear
...
* First-stage growth residualization - now truly at firm level
