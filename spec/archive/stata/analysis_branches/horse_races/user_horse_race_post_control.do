*====================================================================*
* user_horse_race_post_control.do
*
* Canonical horse-race design where competing channels enter as
...
local specname "user_horse_race_post_control_`fe_mode'_`panel_variant'"
capture log close
cap mkdir "$LOG_DIR"
...
gen double hr_growth_post_s = covid * tile_post_c * startup
gen double hr_eq_post = covid * eq_any_firm
gen double hr_eq_post_s = covid * eq_any_firm * startup
...
foreach s of local specs {
    ...
    local exog "var4"
    if "`s'" == "firm_growth" {
        local exog "var4 hr_growth_post hr_growth_post_s"
    }
    else if "`s'" == "equity_comp" {
        local exog "var4 hr_eq_post hr_eq_post_s"
    }
    else if "`s'" == "firm_growth_equity_comp" {
        local exog "var4 hr_growth_post hr_growth_post_s hr_eq_post hr_eq_post_s"
    }
