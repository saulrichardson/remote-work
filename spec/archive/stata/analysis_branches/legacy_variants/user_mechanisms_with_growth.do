...
*  • Adds an equity-offer control path from LLM postings:
*      - Equity-only control column
*      - Growth + equity combined column
...
// Equity-only control
local ols_exog_equity    "var4 var19 var20"
local iv_exog_equity     "var4 var19 var20"
local instr_equity       "var6 var7"
local endo_equity        "var3 var5"

// Growth (endogenous) + equity controls
local ols_exog_gendog_eq "var4 var17_e var18_e var19 var20"
local iv_exog_gendog_eq  "var4 var17_e var18_e var19 var20"
local instr_gendog_eq    "var6 var7"
local endo_gendog_eq     "var3 var5"
...
foreach s in `specs' {
    ...
    if "`s'" == "baseline" {
        ...
        local OLS_EXOG  "`ols_exog_equity'"
        local IV_EXOG   "`iv_exog_equity'"
        local INSTR     "`instr_equity'"
        local ENDO      "`endo_equity'"
    }
    else if "`s'" == "growth_endog_equity" {
        local OLS_EXOG  "`ols_exog_gendog_eq'"
        local IV_EXOG   "`iv_exog_gendog_eq'"
        local INSTR     "`instr_gendog_eq'"
        local ENDO      "`endo_gendog_eq'"
    }
    else if "`s'" == "hhi" {
