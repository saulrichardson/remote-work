*============================================================*
* firm_scaling_crunchbase_fundraising_followups_pure.do
*
* Purpose
*   Meeting follow-ups version of the Crunchbase fundraising regression that
*   runs the *pure* spec (Remote×Post only; NO Startup×Post control):
*
*     y = beta * (Remote × Post) + FE_firm + FE_time + e
*     IV: (Remote × Post)  <-  (Teleworkable × Post)
*
*   This is a narrow, transcript-driven spec. It mirrors
*   firm_scaling_crunchbase_fundraising_followups.do, but removes var4 entirely.
*
...
*
* Outputs
*   results/raw/firm_scaling_crunchbase_fundraising_followups_pure/
...
local specname "firm_scaling_crunchbase_fundraising_followups_pure"
capture log close
cap mkdir "$LOG_DIR"
...
// 2) Required variables (pure spec does NOT require var4)
local required_vars ///
...
// Keep only Crunchbase-matched observations
keep if cb_matched == 1

// Sanity: confirm Crunchbase outcomes are not missing for matched rows
...
// Derived intensive-margin outcome
...
// Firm-level ever-round indicator (within panel window)
tempvar __max_round
bys firm_id_num: egen `__max_round' = max(cb_round_count)
gen byte ever_round = (`__max_round' > 0) if !missing(`__max_round')
drop `__max_round'

// Geography: HQ in New York, NY or San Francisco, CA
tempvar __hqcity __hqstate
gen strL `__hqcity' = lower(trim(hqcity))
gen str2 `__hqstate' = upper(trim(hqstate))
gen byte hq_ny_sf = (`__hqcity' == "new york" & `__hqstate' == "NY") | (`__hqcity' == "san francisco" & `__hqstate' == "CA")
...
// 4) Baseline policy: drop public firms (public as-of-today filter)
...
// Helper: run single-endog (var3) pure spec on current sample
program define __run_single_endog_pure
    ...
    post handle_reg ("`sample_tag'") ("`spec_tag'") ("OLS") ("`y'") ("var3") ///
        (`b') (`se') (`pval') (`pre_mean') ///
        (.) (.) (`N')
    ...
    local t    = `b'/`se'
    local pval = 2*ttail(e(df_r), abs(`t'))
    post handle_reg ("`sample_tag'") ("`spec_tag'") ("IV") ("`y'") ("var3") ///
        (`b') (`se') (`pval') (`pre_mean') ///
        (`rkf') (`F3') (`N')
    ...
    local t    = `b'/`se'
    local pval = 2*ttail(e(df_r), abs(`t'))
    post handle_fs ("`sample_tag'") ("`spec_tag'") ("`y'") ("var3") ("var6") ///
        (`b') (`se') (`pval') ///
        (`F3') (`rkf') (`N_fs')
end

// Helper: run geo interaction (two endogenous vars: var3 and var3_out)
program define __run_geo_interaction_pure
    syntax, SAMPLE(string) SPEC(string) OUTCOME(name)
    ...
    // OLS
    reghdfe `y' var3 var3_out, absorb(firm_id_num yh_num) vce(cluster firm_id_num)
    local N = e(N)
    foreach p in var3 var3_out {
        local b    = _b[`p']
        local se   = _se[`p']
        local t    = `b'/`se'
        local pval = 2*ttail(e(df_r), abs(`t'))
    ...
    ivreghdfe `y' (var3 var3_out = var6 var6_out), absorb(firm_id_num yh_num) vce(cluster firm_id_num) savefirst
    local rkf = e(rkf)
    local N = e(N)
    matrix FS = e(first)
    local F3 = FS[4,1]
...
foreach y of local outcomes {
    __run_single_endog_pure, sample("matched_private") spec("baseline") outcome(`y')
}
...
foreach cut in 10 20 {
    ...
    foreach y of local outcomes {
        __run_single_endog_pure, sample("matched_private") spec("age_lt`cut'") outcome(`y')
    }
}
...
foreach y of local outcomes {
    __run_single_endog_pure, sample("matched_private") spec("pos_usd_only") outcome(`y')
}
...
foreach y of local outcomes {
    __run_single_endog_pure, sample("matched_private") spec("firms_ever_round") outcome(`y')
}
...
foreach y of local outcomes {
    __run_single_endog_pure, sample("matched_private") spec("hq_ny_sf") outcome(`y')
}
...
foreach y of local outcomes {
    __run_single_endog_pure, sample("matched_private") spec("hq_outside_ny_sf") outcome(`y')
}
...
foreach y of local outcomes {
    __run_geo_interaction_pure, sample("matched_private") spec("geo_interaction") outcome(`y')
}
