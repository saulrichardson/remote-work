*======================================================================*
* user_productivity_msa_interactions.do
* Estimates CSA-style heterogeneous effects at the MSA level.
* The script identifies the top-N MSAs (default: 30) by employment
* size measured in 2019H2, interacts the var5 regressor with each
* MSA dummy, and exports OLS coefficients with clustered SEs.
*======================================================================*
...
local specname user_productivity_msa_interactions_`panel_variant'
...
*---- Import pre-computed top-MSA list --------------------------------*
local msa_list "$PROJECT_ROOT/data/clean/top_msas_`panel_variant'.csv"
if !fileexists("`msa_list'") {
    di as error "Top-MSA list missing: `msa_list'. Run the Python builder first."
    exit 601
}
...
preserve
    ...
    keep cbsacode msa_name msa_rank
    save `top_msas'
restore

merge m:1 cbsacode using `top_msas', keep(match) nogen
if _N == 0 {
    di as error "No observations remain after merging top MSAs."
    exit 2000
}
...
di as text "Estimating MSA-varying effects for " ///
    `:word count `msa_ids'' " MSAs (top `top_n')."
...
di as result "✓ Wrote MSA-specific results → `result_dir'/consolidated_results_by_msa.csv"
capture log close
