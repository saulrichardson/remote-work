...
local specname "firm_scaling_llm_equity_variants"
capture log close
cap mkdir "$LOG_DIR"
...
foreach spec_variant of local spec_variants {
    ...
    foreach y of local outcome_vars {
        ...
        capture quietly reghdfe `y' `ols_rhs', absorb(firm_id yh) vce(cluster firm_id)
        if _rc continue
        local nobs = e(N)
        ...
        capture quietly ivreghdfe `y' (`iv_endo' = `iv_inst') `iv_exog', absorb(firm_id yh) vce(cluster firm_id)
        if _rc continue
        local nobs = e(N)
        local rkf = .
