*============================================================*
* firm_scaling_llm_equity_cb_modes.do
* Two-sample-mode LLM equity analysis for firm growth:
*   1) matched  : cb-eligible + parse-covered firm×yh rows
*   2) backfill : cb-eligible universe with missing LLM equity backfilled to 0
*
* Outputs:
*   - results/raw/firm_scaling_llm_equity_cb_modes/consolidated_results.csv
*   - results/raw/llm_equity_cb_modes/sample_accounting.csv
*   - results/raw/llm_equity_cb_modes/startup_vs_nonstartup_equity_shares.csv
*   - results/raw/llm_equity_cb_modes/intensive_margin_summary.csv
*============================================================*

args llm_panel_csv cb_panel_csv
...
if "`cb_panel_csv'"  == "" local cb_panel_csv  "$processed_data/firm_panel_with_cb.csv"
...
capture confirm file "`cb_panel_csv'"
if _rc {
    di as error "Missing Crunchbase panel CSV: `cb_panel_csv'"
    exit 601
}

local specname "firm_scaling_llm_equity_cb_modes"
capture log close
cap mkdir "$LOG_DIR"
log using "$LOG_DIR/`specname'.log", replace text

* 1) Build analysis panel: baseline firm panel + LLM + CB eligibility
tempfile base_panel llm_panel cb_panel panel_all panel_mode
...
preserve
import delimited using "`cb_panel_csv'", clear varnames(1)
keep companyname yh org_uuid
gen str244 __firm_name_key = lower(trim(companyname))
rename yh __yh_key
gen byte cb_eligible = !missing(org_uuid)
keep __firm_name_key __yh_key cb_eligible
drop if missing(__firm_name_key) | missing(__yh_key)
duplicates drop __firm_name_key __yh_key, force
save `cb_panel', replace
restore

use `base_panel', clear
merge 1:1 __firm_name_key __yh_key using `llm_panel', nogen keep(master match)
merge 1:1 __firm_name_key __yh_key using `cb_panel',  nogen keep(master match)
replace cb_eligible = 0 if missing(cb_eligible)
...
local desc_dir "$results/llm_equity_cb_modes"
...
foreach sample_mode of local sample_modes {
    ...
    gen double eq_any_obs = .
    gen double eq_share_obs = .
    gen double eq_count_obs = .

    if "`sample_mode'" == "matched" {
        replace eq_any_obs = llm_equity_any if eq_has_parse == 1
        replace eq_share_obs = llm_equity_share_parse_ok if eq_has_parse == 1
        replace eq_count_obs = llm_n_equity_true if eq_has_parse == 1
    }
    else if "`sample_mode'" == "backfill" {
        replace eq_any_obs = cond(eq_has_parse == 1, llm_equity_any, 0)
        replace eq_share_obs = cond(eq_has_parse == 1, llm_equity_share_parse_ok, 0)
        replace eq_count_obs = cond(eq_has_parse == 1, llm_n_equity_true, 0)
    ...
    replace eq_share_obs = . if eq_share_obs < 0 | eq_share_obs > 1
    replace eq_count_obs = . if eq_count_obs < 0
