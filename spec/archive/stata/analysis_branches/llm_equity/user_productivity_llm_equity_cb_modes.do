*============================================================*
* user_productivity_llm_equity_cb_modes.do
* Two-sample-mode LLM equity analysis for user productivity:
*   1) matched  : cb-eligible + parse-covered firm×yh rows
*   2) backfill : cb-eligible universe with missing LLM equity backfilled to 0
*
* Output:
*   - results/raw/user_productivity_llm_equity_cb_modes_<panel>/consolidated_results.csv
*============================================================*

args panel_variant llm_panel_csv cb_panel_csv
...
local specname "user_productivity_llm_equity_cb_modes_`panel_variant'"
capture log close
cap mkdir "$LOG_DIR"
log using "$LOG_DIR/`specname'.log", replace text

* 1) Build firm_id × yh signal panel for LLM + CB eligibility
tempfile firm_keys llm_by_name cb_by_name signal_by_firm panel_mode
...
drop if missing(__firm_name_key) | missing(__yh_key)
duplicates drop __firm_name_key __yh_key, force
save `cb_by_name', replace

use `firm_keys', clear
merge 1:1 __firm_name_key __yh_key using `llm_by_name', keep(master match) nogen
merge 1:1 __firm_name_key __yh_key using `cb_by_name', keep(master match) nogen
replace cb_eligible = 0 if missing(cb_eligible)

keep firm_id yh cb_eligible llm_n_parse_ok llm_equity_any llm_equity_share_parse_ok llm_n_equity_true
duplicates drop firm_id yh, force
save `signal_by_firm', replace

* 2) Load user panel and merge LLM + CB signals
