...
do "../src/globals.do"      // defines $processed_data and $results

local specname   "worker_productivity_alternative_fe"
local result_path "$results/`specname'"
capture mkdir "`result_path'"

* ---------- 2. FE keywords & outcomes -------------------------------
local fe_keywords userfirmyh time firm user none 
...
foreach fe of local fe_keywords {
    ...
    * --- map keyword → absorb-list & short tag ----------------------
    if      "`fe'"=="userfirmyh" {
        local absorb "user_id firm_id yh"
        local tag    "ufy"
    }
    else if "`fe'"=="time" {
        local absorb "yh"
