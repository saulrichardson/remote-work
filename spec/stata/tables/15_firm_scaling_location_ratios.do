...
local geo_counts "$processed_data/firm_geography_counts_imputed.csv"
if !fileexists("`geo_counts'") {
    di as error "Missing geography counts file: `geo_counts'"
    exit 601
}

tempfile geo_counts_panel
import delimited using "`geo_counts'", clear varnames(1)
replace companyname = lower(companyname)
save `geo_counts_panel', replace

use `firm_panel', clear
merge 1:1 companyname yh using `geo_counts_panel', keep(master match) nogen
...
*---- Results setup ---------------------------------------------------*
local specname "15_firm_scaling_location_ratios"
capture log close
cap mkdir "$LOG_DIR"
