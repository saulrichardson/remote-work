...
We want a figure that helps interpret the dynamic remote-work effects identified in the active
user-productivity baseline, now represented in the paper-order Stata surface by scripts such as
`spec/stata/tables/01_user_productivity_precovid_total_ols_single.do`. Two complementary approaches are on the table:
...
4. For a binscatter-style visual that keeps the same fixed effects/controls, run `spec/archive/stata/scratch/binsreg_var5.do` to generate the binsreg residual figure or re-plot the saved data via `src/py/plot_binsreg_var5_from_savedata.py`.
